      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-500 mt-1">
            已收集水源地数据整理录入（来源 U盘「收集报告」库 ws_sources）。无坐标水源地已录入档案，待补坐标后可在地图显示。
          </p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-100 text-gray-600 text-left">
                  ...
                  <th className="px-3 py-2.5 font-medium">水源类型</th>
                  <th className="px-3 py-2.5 font-medium">坐标</th>
                  <th className="px-3 py-2.5 font-medium">平台边界</th>
                  <th className="px-3 py-2.5 font-medium">平台点位</th>
                  <th className="px-3 py-2.5 font-medium">档案状态</th>
                  <th className="px-3 py-2.5 font-medium">批复文号 / 来源</th>
                </tr>
              </thead>
              <tbody>
                {list.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2 text-gray-600">{r.waterType || '—'}</td>
                      <td className="px-3 py-2">
                        <span
                          className={`px-1.5 py-0.5 rounded text-[11px] font-medium ${
                            r.coordStatus === '有' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'
                          }`}
                        >
                          {r.coordStatus === '有' ? '有' : '无'}
                        </span>
                      </td>
                      <td className="px-3 py-2 text-gray-600">{r.platformBoundary}</td>
                      <td className="px-3 py-2 text-gray-600">{r.platformPoint}</td>
                      <td className="px-3 py-2">
                        <span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${st.bg} ${st.text}`}>
                          {st.label}
                        </span>
                      </td>
                      <td className="px-3 py-2 text-xs text-gray-500 max-w-[220px] truncate" title={r.approvalNo}>
                        {r.approvalNo || '—'}
                      </td>
                    </tr>
                  );
        ...
        <div className="mt-4 text-xs text-gray-400">
          说明：「待补坐标」水源地已录入档案，坐标补全后可通过「归档水源地新增模板」接入地图水井/归档边界图层。
        </div>
      </div>
    </div>
  );
