...
lines = read_file(os.path.join(BASE, 'pages', 'Dashboard.tsx'))

# ============================================================
# 1. 提取编码统计 IIFE (lines 161-209)
# ============================================================
# The IIFE is: {loaded && (() => { ... })()}
code_stats_lines = lines[160:209]  # 0-based: 160 = line 161

code_stats_content = '\n'.join(code_stats_lines)
# Replace `loaded` with props.loaded, `sources` with props.sources, `batchGenerateCodes` import
code_stats_component = """import React from 'react';
...
"""
# Extract the IIFE body (remove the outer {loaded && (() => { and })()})
# Find the IIFE content between the outer wrapper
# The content starts with "loaded &&" and wraps in (() => { ... })()
# Let's just include it as-is, replacing `loaded` checks

# Actually, let me just take the inner content of the IIFE
# Lines 161-209 in the file:
# {loaded &&
#   (() => {
...
# Extract from line 163 (const codeMap...) to line 208 (the closing of the render)
iife_body = '\n'.join(lines[162:208])
...
write_file(os.path.join(BASE, 'components', 'dashboard', 'CodeStatsPanel.tsx'),
           code_stats_component)
print(f"CodeStatsPanel: {len(code_stats_component.split(chr(10)))} lines")

# ============================================================
# 2. 提取保护区统计 (lines 372-987)
# ============================================================
# This is the big IIFE block
zone_stats_lines = lines[371:987]  # 0-based: 371 = line 372
zone_stats_content = '\n'.join(zone_stats_lines)
...
#   {/* 分隔标题 */}
#   ...
#   {zoneResults.length === 0 ? (...) : (zoneResults.length > 0 && (() => { ... })())}
# </div>

# I need to extract the IIFE body and convert it to a component
# The IIFE starts at the line with "zoneResults.length > 0 && (() => {"
# and ends with "})()"

# Find the IIFE start (around line 410 based on earlier reading)
iife_start = None
iife_end = None
for i, line in enumerate(zone_stats_lines):
    if 'zoneResults.length > 0 &&' in line and '(()' in line:
        iife_start = i + 1  # line after the && (() => {
        break
    if '(() => {' in line and iife_start is None and 'zoneResults' in ''.join(zone_stats_lines[max(0,i-3):i+1]):
        iife_start = i + 1
        break

# Actually, let me find it more precisely by looking for the pattern
for i, line in enumerate(zone_stats_lines):
    if '(() => {' in line:
        iife_start = i + 1
        break

for i in range(len(zone_stats_lines) - 1, -1, -1):
    if '})()' in zone_stats_lines[i]:
        iife_end = i
        break

print(f"IIFE body: lines {iife_start} to {iife_end} (relative)")

iife_body = '\n'.join(zone_stats_lines[iife_start:iife_end])

zone_stats_component = """import React from 'react';
...
write_file(os.path.join(BASE, 'components', 'dashboard', 'ZoneStatsPanel.tsx'),
           zone_stats_component)
print(f"ZoneStatsPanel: {len(zone_stats_component.split(chr(10)))} lines")
