...
## 已完成的优化（截至 2026-08-11）

| 优先级 | 优化项 | 效果 |
|--------|--------|------|
| P0 | 数据加载延迟（Dashboard 骨架屏） | 首屏 TTI 2.25s→0.95s |
| P1 | 系统字体栈（去 Google Fonts） | 消除远程字体加载 |
| P2 | 高频页面空闲预加载 | 页面切换零延迟 |
| P3 | vendor-react 瘦身（拆 jspdf/jszip） | modulepreload 1.03MB→0.60MB |
| P4 | **按需数据切分**（按城市拆分） | 数据页首屏数据 208KB→18.56KB（-91%）；Home 页不再单独加载全量 79KB+129KB，复用 store |

---

## 现状基线（截至 P4.0.0, commit 待定）
