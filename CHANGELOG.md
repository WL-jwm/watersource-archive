...
### 坐标自动校正与转换（GCJ-02 → WGS-84 统一治理）

- **数据全量转 WGS-84 存储**：统一点位库 1002 条、资料包 63 条、归档 64 条 + 6 井、保护区边界 14 面 + 3 边界、河北 11 市行政边界、1258 环保护区边界（含内联数据 3.86MB 重新生成）全部由 GCJ-02 转为 WGS-84，原始数据保留 `.bak_gcj` 备份
- **渲染层按底图转换**：新增 `src/utils/coordTransform.ts`（标准火星坐标算法 wgs2gcj / gcj2wgs / gcj2wgsExact 迭代精化），MapView 统一 `toDisplay` 出口——高德底图（GCJ-02 影像）渲染时转 GCJ-02，天地图（CGCS2000≈WGS-84）原样显示，两套底图点位严格对齐
- **选点写入校正**：地图选点/空间查询点击坐标按底图统一转 WGS-84 存储（高德 gcj2wgs、天地图原样）
- **修复坐标转换迭代 bug**：`gcj2wgsExact` 迭代公式 `lng-(glng-lng)` 改为 `lng-(glng-wlng)`（不动点迭代需在当前迭代值处计算偏移），原公式向输入漂移不收敛（误差 6.5m→0.08m）；同步修复 4 个 Python 批量转换脚本并重转全部数据，反向还原验证全部通过（最大误差 <0.1m）

### 测试
- tsc 0 错误、ESLint 0 errors（剩余 9 项既有 warning 非本次引入）、vitest 1281 全过（新增 utilsCoordTransform 9 例：已知点/往返一致/精化优于一阶/境外边界/点位对象）
- 浏览器实测：高德底图岗南水库标记贴岸边、天地图影像下标记位于水库水域（两套底图对齐无偏移）、天地图切换正常（tk+sk）、选点弹窗显示 WGS-84 坐标均通过

## [P8.13.0] - 2026-08-26

### 批量核实井位坐标：地图选点 + Excel 批量导入

- **地图选点落点**：归档页「待核实坐标」每条新增「地图选点」按钮，跳转地图进入选点模式（自动切天地图高清影像）
  - 点击地图即落临时标记，弹出精确坐标确认框，一键「确认核实」写入并实时上地图（绿色标记）
  - 浮动面板显示选点目标/状态，支持退出选点、返回归档页继续核实
- **Excel 批量导入**：新增「下载批量模板」（预填 52 条待补坐标名称）与「批量导入」（选择 .xlsx 一键导入）
  - 模板列：水源地名称/精确经度/精确纬度/备注；导入按名称匹配待补坐标档案，带坐标自动标记已核实
  - 导入结果提示成功/缺坐标/未匹配明细，未匹配项不污染
- waitCoordStore 新增 `batchImport` 批量导入接口（localStorage 持久化）
- 修复既存 bug：Leaflet `TileLayer.setMaxZoom` 不存在导致底图切换运行时崩溃（卫星/天地图按钮均受影响），改为 `options.maxZoom` 赋值
- 修复 waitCoordStore `resetAll` 未调用 `set()` 导致内存态未清空的问题

### 测试
- tsc 0 错误、ESLint 0 问题、vitest 1272 全过（新增 waitCoordStore 6 例）
- 浏览器实测：地图选点（点击→确认→绿色标记）、批量导入（3 条匹配 + 1 条未匹配）均通过

## [P8.12.0] - 2026-08-24

### 待补坐标核实闭环：已核实井位实时上地图

- 归档页「待核实坐标」在线核实的精确井位，**实时在地图显示**为绿色标记（新增「已核实井位」图层）
- 与「资料包点位（青色近似）」「水井（棕色精确）」区分，形成「档案→核实→地图」完整闭环
- 已核实井位弹窗显示精确坐标/备注/核实时间，开关按钮实时显示已核实数量
- 数据来自 waitCoordStore（localStorage），核实保存后地图即时更新
...
- 底图新增「天地图」选项：天地图影像底图（img）+ 影像注记（cia），国产权威高清影像
- 瓦片走天地图 WMTS（Web Mercator），tk 密钥独立配置于 `src/data/tiandituConfig.ts`，支持到 18 级
- 应用开启了安全密钥(sk)校验，瓦片请求需同时携带 tk（应用 Key）+ sk（安全密钥），否则返回 301020；已实测 HTTP 200 正常加载
- 底图切换扩展为「标准 / 卫星 / 天地图」三项
- 天地图访问需域名白名单（公司局域网 IP 192.168.1.21）
...
- 地图最大缩放级别由 14 提升至 **19**（卫星影像支持放大到 19 级查看细节）
- 卫星影像与注记瓦片 URL 增加 **scale=2** 高清参数，影像更清晰
- 标准底图同步加 scale=2 高清，并保持 maxZoom 18
- 切换卫星/标准时动态调整底图最大缩放（卫星 19 / 标准 18）
...
- 「待核实坐标」Tab 新增「导出已核实坐标」按钮，把已核实的精确坐标导出为 Excel（水源地名称/地区/精确经纬度/备注/核实时间）
- 无已核实数据时提示，不导出空表
